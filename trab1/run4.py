    ...
    while _ils_has_better_order:
        ...
        for shake in shakes:
            ...
            if _ils['cost'] > _ils_better_neigh_cost:
                if _debug>0: print "ILS improve cost, ( %d -> %d) %d" % (_ils_best_cost, _ils['cost'], _shake_count)
                _ils_better_neigh_order = _ils['order']
                _ils_better_neigh_cost = _ils['cost']
                _ils_has_better_order = True
                _shake_count =0
        if _shake_count > SHAKE_THRESHOLD:
            print "got shake threashold"
            break
        if not _ils_has_better_order:
            if _debug >0: print "no more improves on ILS, cost (%d -> %d) %d" % (_ils_startcost, _ils_best_cost, _shake_count)
        _ils_runs+=1
